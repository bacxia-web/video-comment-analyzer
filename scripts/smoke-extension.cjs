...
(async () => {
  ...
  try {
    ...
    await settings.locator('#keyDialog').waitFor();
    assert.equal(await settings.locator('#keyDialog input').count(),1);
    const [signup] = await Promise.all([settings.waitForEvent('popup'),settings.locator('#keyDialog a').click()]);
    ...
    await settings.locator('#setupKey').fill('fixture-first-key');
    await settings.locator('#setupForm button[type=submit]').click();
    await settings.locator('#keyDialog').waitFor({state:'detached'});
    await settings.waitForFunction(()=>document.getElementById('deepseekKey').value==='fixture-first-key');
    await settings.reload(); assert.equal(await settings.locator('#keyDialog').count(),0);
    ...
    console.log('PASS first-use single-key dialog, application hyperlink, local persistence and settings edits');
