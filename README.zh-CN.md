...
小红书评论支持实时获取进度、分批完整分析、议题统计和精选评论。获取时可随时停止，分析失败后可重试，无需重新获取。

视频摘要基于字幕，不识别画面，暂不支持无字幕视频。

## 安装

1. [下载安装包](https://raw.githubusercontent.com/bacxia-web/video-comment-analyzer/main/releases/video-comment-analyzer-v0.2.5.zip)，解压到一个长期保留的文件夹。
...
「小红书评论获取设置」可调整评论数量上限、滚动轮数和连续无新增后的停止轮数。默认分别为 1,000 条、20 轮和 3 轮。调高会增加等待、页面操作及分析用量，不代表更安全；设置页提供具体范围和风险说明。
