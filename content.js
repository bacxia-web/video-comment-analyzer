...
function tryInjectNoteButton() {
  ...
  function attempt() {
    ...
    if (attempts >= maxAttempts) {
      debugLog(
        "[YouTube Panorama Content] Player container not found after retries, giving up",
      );
      if (ytdNoteButtonRetryTimer) {
        clearInterval(ytdNoteButtonRetryTimer);
        ytdNoteButtonRetryTimer = null;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  debugLog("[YouTube Panorama Content] Received message:", message.action, message);

  if (message.action === "getVideoInfo") {
    // Read video title and channel name from the page
    const info = extractVideoInfo();
    debugLog("[YouTube Panorama Content] Returning video info:", info);
    sendResponse(info);
  ...
  if (message.action === "seekTo") {
    ...
    debugLog("[YouTube Panorama Content] Seeking to:", message.seconds);
  ...
  debugLog("[YouTube Panorama Content] Unknown action:", message.action);
 ...
 *
 * When clicked, it opens the YouTube Panorama side panel.
 */
...
function createDigestButton() {
  ...
  digestButton.setAttribute("aria-label", "Open YouTube Panorama");
  ...
  digestButton.addEventListener("click", async (e) => {
    ...
    debugLog("[YouTube Panorama] Digest button clicked");
    ...
    try {
      ...
      debugLog("[YouTube Panorama] openSidePanel response:", result);
    } catch (err) {
      console.error("[YouTube Panorama] Failed to open side panel:", err);
    }
  });
...
function injectDigestButton() {
  ...
  if (!actionsContainer) {
    debugLog("[YouTube Panorama Content] Visible actions container not found yet");
    return false;
  }
  ...
  debugLog("[YouTube Panorama Content] Digest button reconciled");
  return true;
}
...
function injectNoteButton() {
  ...
  if (!playerContainer) {
    debugLog(
      "[YouTube Panorama Content] Player container not found yet, will retry",
    );
    return;
  }
  ...
  debugLog("[YouTube Panorama Content] Injecting note button");
  ...
  debugLog("[YouTube Panorama Content] Note button injected");
}
...
async function saveCurrentNote() {
  debugLog("[YouTube Panorama] Saving note");

  const video = document.querySelector("video.html5-main-video");
  if (!video) {
    console.error("[YouTube Panorama] No video element found");
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      console.error("[YouTube Panorama] Save note error:", result.error);
    }
  } catch (err) {
    if (noteButton) {
      noteButton.innerHTML =
    ...
    console.error("[YouTube Panorama] Save note exception:", err);
  }
...
function seekToTimestamp(seconds) {
  const video = document.querySelector("video.html5-main-video");
  if (!video) {
    console.error("[YouTube Panorama Content] No video element found for seek");
    return;
  }

  debugLog("[YouTube Panorama Content] Seeking to:", seconds);
