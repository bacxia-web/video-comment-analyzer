...
function tryInjectNoteButton() {
  ...
  function attempt() {
    ...
    if (attempts >= maxAttempts) {
      debugLog(
        "[Video & Comment Analyzer Content] Player container not found after retries, giving up",
      );
      if (ytdNoteButtonRetryTimer) {
        clearInterval(ytdNoteButtonRetryTimer);
        ytdNoteButtonRetryTimer = null;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  debugLog("[Video & Comment Analyzer Content] Received message:", message.action, message);

  if (message.action === "getVideoInfo") {
    // Read video title and channel name from the page
    const info = extractVideoInfo();
    debugLog("[Video & Comment Analyzer Content] Returning video info:", info);
    sendResponse(info);
  ...
  if (message.action === "seekTo") {
    ...
    debugLog("[Video & Comment Analyzer Content] Seeking to:", message.seconds);
  ...
  debugLog("[Video & Comment Analyzer Content] Unknown action:", message.action);
 ...
 *
 * When clicked, it opens the Video & Comment Analyzer side panel.
 */
...
function createDigestButton() {
  ...
  digestButton.addEventListener("click", async (e) => {
    ...
    debugLog("[Video & Comment Analyzer] Digest button clicked");
    ...
    try {
      ...
      debugLog("[Video & Comment Analyzer] openSidePanel response:", result);
    } catch (err) {
      console.error("[Video & Comment Analyzer] Failed to open side panel:", err);
    }
  });
...
function injectDigestButton() {
  ...
  if (!actionsContainer) {
    debugLog("[Video & Comment Analyzer Content] Visible actions container not found yet");
    return false;
  }
  ...
  debugLog("[Video & Comment Analyzer Content] Digest button reconciled");
  return true;
}
...
function injectNoteButton() {
  ...
  if (!playerContainer) {
    debugLog(
      "[Video & Comment Analyzer Content] Player container not found yet, will retry",
    );
    return;
  }
  ...
  debugLog("[Video & Comment Analyzer Content] Injecting note button");
  ...
  debugLog("[Video & Comment Analyzer Content] Note button injected");
}
...
async function saveCurrentNote() {
  debugLog("[Video & Comment Analyzer] Saving note");

  const video = document.querySelector("video.html5-main-video");
  if (!video) {
    console.error("[Video & Comment Analyzer] No video element found");
    return;
  }
...
function seekToTimestamp(seconds) {
  const video = document.querySelector("video.html5-main-video");
  if (!video) {
    console.error("[Video & Comment Analyzer Content] No video element found for seek");
    return;
  }

  debugLog("[Video & Comment Analyzer Content] Seeking to:", seconds);
