const YTD_OPTIONS = (() => {
  ...
  const COPY = {
    en: {
      ...
      lede:
        "Keys stay in this Chrome profile and are sent only to Supadata, Google, and DeepSeek. This open-source extension has no developer server or analytics.",
      ...
      supadataQuota: "Free plan: 100 credits per month, no credit card required. Each native transcript request costs 1 credit, including requests returning no transcript. Unused credits do not roll over. This extension uses native transcripts only. ",
      ...
      commentsProvider: "Comments provider",
      ...
      youtubeQuota: "Default free quota for comment requests: 10,000 units per day per Google Cloud project. One page of up to 100 top-level comments costs 1 unit. More pages and replies consume additional units; other API methods may share this quota. ",
      ...
      deepseekHelp:
        "Video & Comment Analyzer uses DeepSeek Flash for overviews, explanations, translation, and note polishing. ",
      ...
      saving: "Saving…",
      saved: "Saved. Reopen Video & Comment Analyzer to use these settings.",
      ...
      resetConfirm:
        "Delete API keys, cached digests, comment analyses, translations, and saved notes from this Chrome profile?",
    ...
    "zh-CN": {
      ...
      lede:
        "密钥仅保存在当前 Chrome 个人资料中，只会发送给 Supadata、Google 和 DeepSeek。本开源扩展没有开发者服务器，也不使用分析服务。",
      ...
      supadataQuota: "免费方案：每月 100 积分，无需信用卡。每次原生字幕请求消耗 1 积分，约可请求 100 次；未找到字幕的请求也可能扣除积分。额度不结转，本插件不会自动使用语音转写。",
      officialPricing: "官方定价与额度 ↗",
      officialQuota: "官方额度说明 ↗",
      requestCost: "请求用量说明 ↗",
      quotaChecked: "额度信息核实于 2026-09-21，以服务商账户显示为准；以上额度不包含 DeepSeek 的 AI 分析费用。",
      commentsProvider: "评论服务",
      ...
      youtubeQuota: "默认免费配额：每天 10,000 单位／Google Cloud 项目。读取一页评论消耗 1 单位，最多返回 100 条顶层评论；翻页和补充回复会继续消耗额度。同一个项目下的其他接口也可能共用此配额。",
      aiProvider: "AI 服务",
      providerSummaryLabel: "支持的 AI 服务",
      providerBadge: "当前版本支持",
      deepseekApiKeyLabel: "DeepSeek API 密钥",
      deepseekHelp:
        "Video & Comment Analyzer 使用 DeepSeek Flash 生成概览、解释内容、翻译字幕和润色笔记。",
      ...
      saving: "正在保存…",
      saved: "已保存。请重新打开 Video & Comment Analyzer 以使用这些设置。",
      ...
      resetConfirm:
        "要从当前 Chrome 个人资料中删除 API 密钥、缓存摘要、评论分析、翻译和已保存的笔记吗？",
