...
test("release copy documents current scope without em dashes", () => {
  ...
  assert.equal(manifest.name, "YouTube Panorama");
  assert.equal(packageJson.name, "youtube-panorama");
  assert.match(read("scripts/package-extension.sh"), /youtube-panorama-v\$version\.zip/);
  assert.doesNotMatch(
    [readme, chineseReadme, read("PRIVACY.md"), read("SECURITY.md")].join("\n"),
    /\bYT Digest\b/,
  );
  assert.match(readme, /^# YouTube Panorama$/m);
  assert.match(
    readme,
  ...
  assert.match(
    readme,
    /permanent folder I choose[\s\S]*tell me its exact full path[\s\S]*If I need a suggestion during this first installation[\s\S]*`~\/Documents\/youtube-panorama`[\s\S]*`%USERPROFILE%\\Documents\\youtube-panorama`[\s\S]*do not assume either path/,
  );
  assert.match(
    readme,
  ...
  assert.match(
    readme,
    /independent derivative of \[YouTube Digest\]\(https:\/\/github\.com\/zarazhangrui\/youtube-digest\)/i,
  );
  assert.doesNotMatch(readme, /^## Contributing$/m);
  assert.match(chineseReadme, /^# YouTube Panorama$/m);
  assert.match(chineseReadme, /把每个 YouTube 视频变成一份可以深入学习的资料/);
  assert.match(chineseReadme, /^## 让你的编程 Agent 帮你安装$/m);
  assert.match(
    chineseReadme,
    /我选择的长期保留文件夹[\s\S]*告诉我准确的完整路径[\s\S]*第一次安装时需要位置建议[\s\S]*`~\/Documents\/youtube-panorama`[\s\S]*`%USERPROFILE%\\Documents\\youtube-panorama`[\s\S]*不要假设我一定使用这些路径/,
  );
  assert.match(
    chineseReadme,
  ...
  assert.match(
    chineseReadme,
    /\[YouTube Digest\]\(https:\/\/github\.com\/zarazhangrui\/youtube-digest\) 的独立衍生版本/,
  );
  ...
  assert.match(
    optionsPage,
    /class="customization-steps"[\s\S]*Open the extracted YouTube Panorama project folder in your coding[\s\S]*Replace \[PROVIDER\] and \[MODEL\][\s\S]*Never include API keys[\s\S]*<\/ol>/,
  );
  assert.match(
    optionsPage,
    /class="prompt-reminder"[\s\S]*Before copying, replace \[PROVIDER\] and \[MODEL\]/,
  );
  assert.doesNotMatch(optionsPage, /~\/Documents\/youtube-panorama/);
  assert.doesNotMatch(optionsPage, /%USERPROFILE%\\Documents\\youtube-panorama/);
  ...
  assert.match(
    readme,
    /first open the exact YouTube Panorama project folder that Chrome loaded through \*\*Load unpacked\*\* in your coding agent/,
  );
  assert.match(
    chineseReadme,
    /先在编程 Agent 中打开 Chrome 通过“加载已解压的扩展程序”使用的那个准确的 YouTube Panorama 项目文件夹/,
  );
